	...
	synchGridToDisplay : function(){
		for(let x = 0; x < WIDTH; x++){
			for(let y = 0; y < HEIGHT; y++){
				...
				else PS.color(x, y, PS.COLOR_BLACK);
			}
		}
...
updateGame = function(step = false){
	...
	for(let x = 0; x < WIDTH; x++){
		...
		for(let y = 0; y < HEIGHT; y++){
			...
			if(left > 0){
				//left
				if (PS.color(left, y) == PS.COLOR_BLACK) neighbors += 1;
				if(up > 0){
					//top left
					if (PS.color(left, up) == PS.COLOR_BLACK) neighbors += 1;
				}
				if(down < HEIGHT){
					//bottom left
					if (PS.color(left, down) == PS.COLOR_BLACK) neighbors += 1;
				}
			}
			if(right < WIDTH){
				//right
				if (PS.color(right, y) == PS.COLOR_BLACK) neighbors += 1;
				if(up > 0){
					//top right
					if (PS.color(right, up) == PS.COLOR_BLACK) neighbors += 1;
				}
				if(down < HEIGHT){
					//bottom right
					if (PS.color(right, down) == PS.COLOR_BLACK) neighbors += 1;
				}
			}
			if(up > 0){
				//above
				if (PS.color(x, up) == PS.COLOR_BLACK) neighbors += 1;
			}
			if(down < HEIGHT){
				//below
				if (PS.color(x, down) == PS.COLOR_BLACK) neighbors += 1;
			}

			//follow rules depending on number of neighbors
			thisCell = PS.color(x, y) == PS.COLOR_BLACK;
...
PS.init = function( system, options ) {
	...
	GRID.createEmpty2DArray(HEIGHT, WIDTH);
	...
	PS.debug("Controls: Mouse, Space, and Delete.\n\nPlay around and have fun!\nMade by Andre Mata Assis.");
};
...
PS.enter = function( x, y, data, options ) {
	...
	if(clicking == true){
		...
		else if (PS.color(x, y) == PS.COLOR_BLACK && !placing) {
			PS.color(x, y, PS.COLOR_WHITE);
			PS.audioPlay("fx_pop");
			GRID.updateGrid(x, y, 0);
		}
